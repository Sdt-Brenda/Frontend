...
export class Estudio extends Component {
    constructor(props) {
        super(props)
    ...
    showModal(id_estudio) {
        this.setState({
            modal: true,
            idToDelete: id_estudio
        })
    }
    ...
    handleClickDelete() {
        if (this.state.idToDelete) {
            ...
            fetch(`http://localhost:8080/api/estudio/${this.state.idToDelete}`, parametros)
                .then(res => {
                    return res.json()
                        .then(body => (
                            {
    ...
    renderAcciones = (rowData) => {
        return (
            <>
                ...
                <Link to={`/estudio/informe/${rowData.id_estudio}`} className="btn btn-primary">
                    <span className="material-symbols-outlined">article</span>
                </Link>
                <button className="btn btn-danger" onClick={() => this.showModal(rowData.id_estudio)}>
        ...
        const dataForDataTable = this.state.estudio.map((estudio, index) => {
            ...
            return (
                ...
                <Modal show={this.state.modal} onHide={this.closeModal}>
                    ...
                    <Modal.Body>¿Está seguro que desea eliminar este estudio?</Modal.Body>
                    <Modal.Footer>
